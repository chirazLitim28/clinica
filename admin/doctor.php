    ...
    <body>
      ...
      ?>
        <div class="container mt-5">
          <a href="doctor-add.php" class="btn btn-dark">Add New Doctor</a>
          <!-- search -->
          ...
          $doctors = getAllDoctors($conn); // Assuming you have an array of doctors

          // Code to fetch doctors and populate the $doctors array goes here

          $total_records = count($doctors); // Total number of records
          $records_per_page = 3; // Number of records to display per page

          $pagination = new Pagination($total_records, $records_per_page);

          $page_url = 'http://localhost/clinic_management31/admin/doctor.php'; // Replace with the actual URL of your PHP file
          $current_page = $pagination->getCurrentPage();

          $doctors_subset = array_slice($doctors, ($current_page - 1) * $records_per_page, $records_per_page);
          ?>
          ...
          <div class="table-responsive">
            <table>
              ...
              <tbody>
                <?php foreach ($doctors_subset as $doctor) { ?>
                  <tr>
                    <!-- <th scope="row">1</th> -->
                    ...
                    <td>
                      ...
                      foreach ($specialties as $specialty) {
                        $g_temp = getSpecialtyById($specialty, $conn);
                        if ($g_temp != 0)
                          $g .= $g_temp['specialty'] . ', ';
                      }
                      echo $g;
                    ...
                    <td>
                      <a href="doctor-edit.php?doctor_id=<?= $doctor['doctor_id'] ?>" class="btn btn-warning">Edit</a>
                      <a href="doctor-delete.php?doctor_id=<?= $doctor['doctor_id'] ?>" class="btn btn-danger">Delete</a>
                    </td>
                  </tr>
                <?php } ?>
              </tbody>
