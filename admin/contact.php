    ...
    <body>
      ...
      ?>
        <div class="container mt-5">
          ...
          $contacts = getAllContacts($conn); // Assuming you have an array of contacts

          // Code to fetch contacts and populate the $contacts array goes here

          $total_records = count($contacts); // Total number of records
          $records_per_page = 2; // Number of records to display per page

          $pagination = new Pagination($total_records, $records_per_page);

          $page_url = 'http://localhost/clinicManagement/admin/contact.php'; // Replace with the actual URL of your PHP file
          $current_page = $pagination->getCurrentPage();

          $contacts_subset = array_slice($contacts, ($current_page - 1) * $records_per_page, $records_per_page);
          ?>
          ...
          <div class="table-responsive">
            <table>
              ...
              <tbody>
                <?php foreach ($contacts_subset as $contact) { ?>
                  <tr>
                    <!-- <th scope="row">1</th> -->
                  ...
                  <td>
                      <a href="contact-delete.php?contact_id=<?= $contact['contact_id'] ?>" class="btn btn-danger">Delete</a>
                    </td>
                  </tr>
                <?php } ?>
              </tbody>
